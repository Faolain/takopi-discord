...
class VoiceManager:
    ...
    async def join_channel(
        ...
        # Connect to the voice channel using VoiceRecvClient for receiving audio
        voice_client: VoiceRecvClient = await voice_channel.connect(cls=VoiceRecvClient)
        ...
        # Create a callback-based sink for receiving audio
        def on_audio(user: discord.User | None, data: VoiceData) -> None:
            if user is not None:
                # Get PCM data from VoiceData
                self.receive_audio(guild_id, user.id, data.pcm)
